...
function registerDownloadsIpcHandlers() {
  ...
  ipcMain.handle('downloads:confirm-overwrite', async (_event, payload = {}) => {
    ...
    const title = String(payload?.title || '').trim() || 'File already exists'
    const message = String(payload?.message || '').trim() || `The file "${fileName}" already exists.`
    const detail = String(payload?.detail || '').trim() || 'Do you want to replace the existing file?'
    const replaceLabel = String(payload?.replaceLabel || '').trim() || 'Replace'
    const keepLabel = String(payload?.keepLabel || '').trim() || 'Do not replace'
    const cancelLabel = String(payload?.cancelLabel || '').trim() || 'Cancel'
    ...
    const result = await dialog.showMessageBox(ownerWindow, {
      ...
      message,
      detail,
      buttons: [replaceLabel, keepLabel, cancelLabel],
      defaultId: 2,
      cancelId: 2,
      noLink: true,
    })

    const selectedIndex = Number(result?.response)
    const action = selectedIndex === 0
      ? 'replace'
      : (selectedIndex === 1 ? 'keep' : 'cancel')
